...
class _FakeLoader(GetAttr):
    _auto_collation, collate_fn, drop_last, = False, noops, False
    dataset_kind, _index_sampler = _DatasetKind.Iterable, Inf.count
...
_collate_types = (np.ndarray, Tensor, typing.Mapping, str)
...
class DataLoader(GetAttr):
    ...
    _methods =  "wif before_iter create_batches sampler create_item after_item "
    _methods += "before_batch create_batch retain after_batch after_iter get_idxs"
    _methods = _methods.split()
    _default = "dataset"

    def __init__(self, dataset=None, bs=None, shuffle=False, drop_last=False,
            indexed=None, num_workers=0, pin_memory=False, timeout=0, n=None, **kwargs):
    ...
    def new(self, dataset):
        kwargs = dict(bs=self.bs, shuffle=self.shuffle, drop_last=self.drop_last,
                indexed=self.indexed, num_workers=self.nw,
                pin_memory=self.pin_memory, timeout=self.timeout)
